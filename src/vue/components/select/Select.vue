<template>
    <div :class="classes" v-clickoutside="handleClose">
        <div :class="selectionClass" ref="reference" @click="toggleMenu">
            <slot name="input">
                <input type="hidden" :name="name" :value="model">
                <div class="removable-label-list">
                    <div class="removable-label border radius small" v-for="(item, index) in selectedMultiple">
                        <span class="removable-label-text">{{ item.label }}</span>
                        <Icon class="removable-label-close-button" type="error" @click.native.stop="removeTag(index)"></Icon>
                    </div>
                </div>
                <span class="flex-box-adapter" :class="['select-placeholder']" v-show="showPlaceholder && !filterable">{{ localePlaceholder }}</span>
                <label class="flex-box-adapter" :class="['select-selected-value']" v-show="!showPlaceholder && !multiple && !filterable"><slot name="selectValue">{{ selectedSingle }}</slot></label>
                <input
                        :id="elementId"
                        type="text"
                        v-if="filterable"
                        ...
                        ref="input"></input>
                <Icon type="error" :class="['select-close']" v-show="showCloseIcon" @click.native.stop="clearSingleSelect"></Icon>
                <Icon type="down" :class="['select-arrow']" v-if="!remote"></Icon>
            </slot>
        </div>
        <transition :name="transitionName">
            <Drop :matchParentWidth="true"
                  :class="dropdownClass"
                  v-show="dropVisible"
                  ...
                  v-transfer-dom>
                <ul v-show="notFoundShow" :class="['select-not-found']"><li>{{ localeNotFoundText }}</li></ul>
                <ul v-show="(!notFound && !remote) || (remote && !loading && !notFound)" :class="['select-dropdown-list']">
                    <slot></slot>
                </ul>
                <ul v-show="loading" :class="['select-loading']">{{ localeLoadingText }}</ul>
            </Drop>
        </transition>
    </div>
</template>
        ...
        computed: {
            ...
            dropdownClass () {
                return [
                    ...
                    'dropdown',
                    {
                        ['select-dropdown-transfer']: this.transfer,
                        ['select-multiple']: this.multiple && this.transfer,
                        ['auto-complete']: this.autoComplete,
                    }];
            },
            selectionClass () {
                return {
                    [`${prefixClass}-selection`]: !this.autoComplete
                };
            },
            showPlaceholder () {
                let status = false
            ...
            localePlaceholder () {
                if (this.placeholder === undefined) {
                    return this.t('core.select.placeholder');
                }
                else {
            ...
            localeNotFoundText () {
                if (this.notFoundText === undefined) {
                    return this.t('core.select.noMatch');
                }
                else {
            ...
            localeLoadingText () {
                if (this.loadingText === undefined) {
                    return this.t('core.select.loading');
                }
                else {
