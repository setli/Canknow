<template>
    <div :class="classes" @mousedown.prevent>
        <div :class="[prefixClass + '-sidebar']" v-if="shortcuts.length">
            <div
                :class="[prefixClass + '-shortcut']"
                v-for="shortcut in shortcuts"
                @click="handleShortcutClick(shortcut)">{{ shortcut.text }}</div>
        </div>
        <div :class="[prefixClass + '-body']">
            <div :class="[prefixClass + '-content', prefixClass + '-content-left']" v-show="!isTime">
                <div :class="[datePrefixClass + '-header']" v-show="leftCurrentView !== 'time'">
                    <span
                        :class="iconButtonClass('prev', '-double')"
                        @click="prevYear('left')"><Icon type="prev"></Icon></span>
                    <span
                        :class="iconButtonClass('prev')"
                        @click="prevMonth"
                        v-show="leftCurrentView === 'Date'"><Icon type="prev"></Icon></span>
                    <date-panel-label
                        :date-panel-label="leftDatePanelLabel"
                        :current-view="leftCurrentView"
                        :date-prefix-cls="datePrefixClass"/>
                    <span
                        :class="iconButtonClass('next', '-double')"
                        @click="nextYear('left')"
                        v-show="leftCurrentView === 'year' || leftCurrentView === 'month'"><Icon type="next"></Icon></span>
                </div>
                <date-table
            ...
            <div :class="[prefixClass + '-content', prefixClass + '-content-right']" v-show="!isTime">
                <div :class="[datePrefixClass + '-header']" v-show="rightCurrentView !== 'time'">
                     <span
                         :class="iconButtonClass('prev', '-double')"
                         @click="prevYear('right')"
                         v-show="rightCurrentView === 'year' || rightCurrentView === 'month'"><Icon type="prev"></Icon></span>
                    <date-panel-label
                    ...
                    <span
                        :class="iconButtonClass('next', '-double')"
                        @click="nextYear('right')"><Icon type="next"></Icon></span>
                    <span
                        :class="iconButtonClass('next')"
                        @click="nextMonth"
                        v-show="rightCurrentView === 'Date'"><Icon type="next"></Icon></span>
                </div>
                <date-table
            ...
            <div :class="[prefixClass + '-content']" v-show="isTime">
                <time-picker
                    ref="timePicker"
                    v-show="isTime"
...
<script>
    ...
    import DateTable from '../base/DateTable.vue';
    import YearTable from '../base/YearTable.vue';
    import MonthTable from '../base/MonthTable.vue';
    import TimePicker from './TimeRange.vue';
    import Confirm from '../base/Confirm.vue';
    import { toDate, prevMonth, nextMonth, initTimeDate, formatDateLabels } from '../util';
    import datePanelLabel from './DatePanelLabel.vue';
    ...
    export default {
        ...
        data () {
            return {
                ...
                datePrefixClass: datePrefixClass,
                shortcuts: [],
                date: initTimeDate(),
                value: '',
                minDate: '',
        ...
        computed: {
            classes () {
                return [
                    `${prefixClass}-body-wrapper`,
                    `${datePrefixClass}-with-range`,
                    {
                        [`${prefixClass}-with-sidebar`]: this.shortcuts.length
                    }
                ];
            },
            leftYear () {
        ...
        watch: {
            value(newVal) {
                ...
                if (this.showTime)
                    this.$refs.timePicker.value = newVal;
            },
            minDate (val) {
                if (this.showTime)
            ...
            maxDate (val) {
                if (this.showTime)
                    this.$refs.timePicker.dateEnd = val;
            },
            format (val) {
                if (this.showTime)
        ...
        methods: {
            panelLabelConfig (direction) {
                const locale = this.t('core.locale');
                const datePanelLabel = this.t('core.datepicker.datePanelLabel');
                const handler = type => {
